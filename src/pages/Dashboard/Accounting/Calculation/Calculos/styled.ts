import { CurrencyDollarSimple, House, Plus } from "phosphor-react";
...
export const AdicionarCalculoButton = styled.button`
  transition: all 500ms ease;
  cursor: ${({ disabled }) => (disabled ? "not-allowed" : "pointer")};
  margin-top: 3rem;
  background-color: ${({ disabled }) =>
    disabled ? theme.colors["gray/300"] : theme.colors.jvrisAqua};
  outline: none;
  border: none;
  text-align: center;
  width: 15rem;
  ...
  :hover {
    background-color: ${({ disabled }) =>
      disabled ? theme.colors["gray/300"] : theme.colors.jvrisAquaDark};
  }
`;

export const RespostaButton = styled.button`
  transition: all 500ms ease;
  cursor: ${({ disabled }) => (disabled ? "not-allowed" : "pointer")};
  margin-top: 3rem;
...
export const RedirectPage = styled.button`
  display: flex;
  justify-content: center;
  align-items: center;
  border: 1px solid ${({ theme }) => theme.colors.gray};
...
export const HomePageIcon = styled(House)`
  font-size: 2rem;
`;
