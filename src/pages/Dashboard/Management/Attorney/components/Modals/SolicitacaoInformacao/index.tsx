...
const SolicitacaoInformacaoModal = () => {
  ...
  const { closeModal, isModalOpen,setShouldReset } = useModalsContext();
  ...
  const onSubmit: SubmitHandler<SubmitSolicitacaoInfo> = async (params) => {
    postInformationRequest({
      ...
      if (response.status == "Created") {
        HotToastSucess("Informação Solicitada com Sucesso");
        setShouldReset(true);
        ...
        HotToastError("Erro ao Solicitar Informação");
      }
    });
  };
