...
const RegistrarAudienciaModal = () => {
  ...
  const onSubmit: SubmitHandler<SubmitRegistrarAudiencia> = (params) => {
    ...
    postAudiences({
      idAto: managing.selectedData.length?  Number(managing.selectedData![0].id): Number(managing.singularSelectedData.id),
      ...
      if (response.status == "Created") {
        toast("Registro de Audiência Realizado com Sucesso", {
          icon: "👏",
          style: {
            borderRadius: "10px",
            background: "#81c784",
        ...
        toast.error("Erro ao Registrar Audiência", {
          icon: "😥",
          style: {
            borderRadius: "10px",
            background: "#e57373",
