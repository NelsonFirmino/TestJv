...
export const PjeClient = (function () {
    function t() {}
    t.ajax = function (t) {
        (t = t || { url: "" }),
...
export const PjeOffice = (function () {
    function PjeOffice() {}
    ...
    const PJEOFFICE_BASE_ENDPOINT =
        PJEOFFICE_PROTOCOL + "://127.0.0.1:" + PJEOFFICE_PORT;
    ...
    const PJEOFFICE_TIMEOUT_ALERT =
        "Alcançado tempo máximo de espera por resposta do PJeOffice Pro (timeout)";
    ...
    const fullRequest = function (method, url, body, apiContext, checkSuccess) {
        ...
        PjeClient.ajax({
            ...
            timeout:
                argument?.REQUEST_TIMEOUT ||
                PJEOFFICE_DEFAULT_ARGUMENTS.REQUEST_TIMEOUT,
            async: true,
            data: body,
            error: function (status, statusText, httpClient) {
                if ("timeout" === status) alert(PJEOFFICE_TIMEOUT_ALERT);
                if (complete) return;
                complete = true;
                if (onUnavailable) onUnavailable(statusText);
            },
            success: function (data, statusText, httpClient) {
                ...
                if (success) {
                    ...
                    if (onFailed) onFailed(statusText);
                }
            }
        });
    };
    ...
    const createBodyRequest = function (taskId, task) {
        return readJson({
            ...
            servidor:
                ...
                PJEOFFICE_DEFAULT_ARGUMENTS.WEB_CONTEXT,
    ...
    PjeOffice.login = function (challengePhrase, apiContext) {
        runTask(
            ...
            {
                enviarPara:
                    apiContext?.argument?.PAGINA_LOGIN ||
                    PJEOFFICE_DEFAULT_ARGUMENTS.PAGINA_LOGIN,
                mensagem: challengePhrase,
                algoritmoAssinatura:
                    apiContext?.argument?.ALGORITMO ||
                    PJEOFFICE_DEFAULT_ARGUMENTS.ALGORITMO_AUTENTICACAO
            },
            apiContext
        );
    };
    ...
    PjeOffice.signHash = function (documents, apiContext) {
        runTask(
            "cnj.assinadorHash",
            {
                algoritmoAssinatura:
                    apiContext?.argument?.ALGORITMO ||
                    PJEOFFICE_DEFAULT_ARGUMENTS.ALGORITMO_ASSINATURA_HASH,
                uploadUrl:
                    apiContext?.argument?.PAGINA_ASSINATURA ||
                    PJEOFFICE_DEFAULT_ARGUMENTS.PAGINA_ASSINATURA,
                modoTeste:
                    apiContext?.argument?.MODO_TESTE ||
                    PJEOFFICE_DEFAULT_ARGUMENTS.MODO_TESTE,
                arquivos: documents
            },
            apiContext
        );
    ...
    PjeOffice.signRemote = function (documents, apiContext) {
        runTask(
            "cnj.assinador",
            {
                modo: "REMOTO",
                padraoAssinatura:
                    apiContext?.argument?.PADRAO_ASSINATURA ||
                    PJEOFFICE_DEFAULT_ARGUMENTS.PADRAO_ASSINATURA,
                tipoAssinatura:
                    apiContext?.argument?.TIPO_ASSINATURA ||
                    PJEOFFICE_DEFAULT_ARGUMENTS.TIPO_ASSINATURA,
                enviarPara:
                    apiContext?.argument?.PAGINA_UPLOAD ||
                    PJEOFFICE_DEFAULT_ARGUMENTS.PAGINA_UPLOAD,
                algoritmoHash:
                    apiContext?.argument?.ALGORITMO ||
                    PJEOFFICE_DEFAULT_ARGUMENTS.ALGORITMO_ASSINATURA,
                arquivos: documents
            },
            apiContext
        );
    ...
    PjeOffice.getCertChain = function (apiContext) {
        runTask(
            ...
            {
                uploadUrl:
                    apiContext?.argument?.PAGINA_CERTCHAIN ||
                    PJEOFFICE_DEFAULT_ARGUMENTS.PAGINA_CERTCHAIN
            },
            apiContext
        );
    };
    ...
    const onMinimumServerSuccess = function (
        apiContext,
        pjeofficeData,
        serverData
    ) {
        if (serverData.parametros_configurados) {
            ...
            const explain =
                ...
                "\n" +
                "M\u00ednima obrigat\u00f3ria: " +
                serverData.versao_pje_office_minima_obrigatoria +
                "\n" +
                "M\u00ednima recomendada: " +
                serverData.versao_pje_office_minima_recomendada +
                "\n" +
                "Mais recente: " +
                serverData.versao_pje_office;
            ...
            if (compare == -1) {
                const failMessage =
                    serverData.versao_pje_office_mensagem_bloqueio + explain;
                alert(failMessage);
            ...
            if (compare == -1) {
                ...
                if (compare == -1) {
                    alert(
                        serverData.versao_pje_office_mensagem_recomendacao +
                            explain
                    );
                }
            }
    ...
    const onAboutSuccess = function (apiContext, pjeofficeData) {
        ...
        const resource =
            argument?.PAGINA_VERSAO_MINIMA ||
            PJEOFFICE_DEFAULT_ARGUMENTS.PAGINA_VERSAO_MINIMA;
        const url =
            window.location.origin +
            PJEOFFICE_DEFAULT_ARGUMENTS.WEB_CONTEXT +
            resource;
        get(url, {
            ...
            onSuccess: function (serverData) {
                //Algum dado foi retornado do servidor
    ...
    PjeOffice.checkMininum = function (apiContext) {
        PjeOffice.about({
            ...
            onUnavailable: function (statusText) {
                alert(
                    "Acesso via certificado digital indispon\u00edvel.\n\n" +
